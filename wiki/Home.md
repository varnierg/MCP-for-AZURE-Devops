...
> * **Azure DevOps does NOT keep a recycle bin or trashcan for work items deleted via the REST API.** If a work item (e.g., Bug, Task, User Story) is deleted via the API (using `call_api` or other means), it is **permanently destroyed** and cannot be restored.
...
* How to use the powerful `call_api` tool.
* Explaining the offline API directory (`search_api_docs` and `get_api_info`) which helps AI models discover and execute arbitrary Azure DevOps REST API requests.
